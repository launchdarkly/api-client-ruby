...
module LaunchDarklyApi
  class ExperimentTimeSeriesSlice
    ...
    def self.openapi_types
      {
        :'time' => :'Integer',
        :'variation_data' => :'Array<ExperimentTimeSeriesVariationSlice>'
      }
    end
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `LaunchDarklyApi::ExperimentTimeSeriesSlice` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `LaunchDarklyApi::ExperimentTimeSeriesSlice`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'variation_data')
        if (value = attributes[:'variation_data']).is_a?(Array)
          self.variation_data = value
        end
      end
    end
    ...
    def ==(o)
      ...
      self.class == o.class &&
          time == o.time &&
          variation_data == o.variation_data
    end
