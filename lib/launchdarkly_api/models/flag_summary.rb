...
module LaunchDarklyApi
  class FlagSummary < ApiModelBase
    ...
    def self.openapi_types
      {
        :'variations' => :'Hash<String, VariationSummary>',
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `LaunchDarklyApi::FlagSummary` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `LaunchDarklyApi::FlagSummary`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }

      if attributes.key?(:'variations')
        if (value = attributes[:'variations']).is_a?(Hash)
    ...
    def list_invalid_properties
      ...
      if @prerequisites.nil?
        invalid_properties.push('invalid value for "prerequisites", prerequisites cannot be nil.')
      end
    ...
    # Custom attribute writer method with validation
    # @param [Object] prerequisites Value to be assigned
    def prerequisites=(prerequisites)
      if prerequisites.nil?
        fail ArgumentError, 'prerequisites cannot be nil'
      end
