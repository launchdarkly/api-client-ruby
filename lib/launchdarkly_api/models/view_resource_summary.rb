...
module LaunchDarklyApi
  class ViewResourceSummary < ApiModelBase
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `LaunchDarklyApi::ViewResourceSummary` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `LaunchDarklyApi::ViewResourceSummary`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def list_invalid_properties
      ...
      if @flag_count.nil?
        invalid_properties.push('invalid value for "flag_count", flag_count cannot be nil.')
      end
    ...
    def valid?
      ...
      return false if @flag_count.nil?
      return false if @segment_count.nil?
      true
    end

    # Custom attribute writer method with validation
    # @param [Object] flag_count Value to be assigned
    def flag_count=(flag_count)
      if flag_count.nil?
        fail ArgumentError, 'flag_count cannot be nil'
      end
