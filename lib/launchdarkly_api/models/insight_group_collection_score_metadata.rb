...
module LaunchDarklyApi
  class InsightGroupCollectionScoreMetadata < ApiModelBase
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `LaunchDarklyApi::InsightGroupCollectionScoreMetadata` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `LaunchDarklyApi::InsightGroupCollectionScoreMetadata`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def list_invalid_properties
      ...
      if @last_period.nil?
        invalid_properties.push('invalid value for "last_period", last_period cannot be nil.')
      end
    ...
    def valid?
      ...
      return false if @last_period.nil?
      true
    end
    ...
    # Custom attribute writer method with validation
    # @param [Object] last_period Value to be assigned
    def last_period=(last_period)
      if last_period.nil?
        fail ArgumentError, 'last_period cannot be nil'
      end
