...
module LaunchDarklyApi
  class DenominatorPost < ApiModelBase
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `LaunchDarklyApi::DenominatorPost` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `LaunchDarklyApi::DenominatorPost`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def hash
      [event_name, is_numeric, data_source, unit_aggregation_type, unit_aggregation_field, filters, window_start_offset, window_end_offset, winsor_lower_percentile, winsor_upper_percentile, winsor_exclude_imputed, winsor_include_imputed].hash
    end
