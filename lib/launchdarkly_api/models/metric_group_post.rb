...
module LaunchDarklyApi
  class MetricGroupPost < ApiModelBase
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `LaunchDarklyApi::MetricGroupPost` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `LaunchDarklyApi::MetricGroupPost`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def list_invalid_properties
      ...
      if @maintainer_id.nil?
        invalid_properties.push('invalid value for "maintainer_id", maintainer_id cannot be nil.')
      end
    ...
    # Custom attribute writer method with validation
    # @param [Object] maintainer_id Value to be assigned
    def maintainer_id=(maintainer_id)
      if maintainer_id.nil?
        fail ArgumentError, 'maintainer_id cannot be nil'
      end
    ...
    def hash
      [key, name, kind, description, maintainer_id, tags, metrics].hash
    end
