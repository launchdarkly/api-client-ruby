...
module LaunchDarklyApi
  class SeriesListRepFloat < ApiModelBase
    ...
    attr_accessor :projected_series
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `LaunchDarklyApi::SeriesListRepFloat` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `LaunchDarklyApi::SeriesListRepFloat`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'projected_series')
        if (value = attributes[:'projected_series']).is_a?(Array)
          self.projected_series = value
        end
      end
    end
    ...
    def ==(o)
      ...
      self.class == o.class &&
          ...
          metadata == o.metadata &&
          series == o.series &&
          projected_series == o.projected_series
    end
    ...
    def hash
      [_links, metadata, series, projected_series].hash
    end
