...
module LaunchDarklyApi
  class FlagImportIntegration < ApiModelBase
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `LaunchDarklyApi::FlagImportIntegration` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `LaunchDarklyApi::FlagImportIntegration`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def valid?
      ...
      integration_key_validator = EnumAttributeValidator.new('String', ["split", "unleash"])
    ...
    def integration_key=(integration_key)
      validator = EnumAttributeValidator.new('String', ["split", "unleash"])
    ...
    def hash
      [_links, _id, integration_key, project_key, config, tags, name, version, _access, _status].hash
    end
