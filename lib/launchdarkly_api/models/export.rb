  ...
  class Export < ApiModelBase
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `LaunchDarklyApi::Export` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `LaunchDarklyApi::Export`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    # Custom attribute writer method with validation
    # @param [Object] initiator Value to be assigned
    def initiator=(initiator)
      if initiator.nil?
        fail ArgumentError, 'initiator cannot be nil'
      end
    ...
    def hash
      [id, segment_key, creation_time, status, size_bytes, size, initiator, _links].hash
    end
