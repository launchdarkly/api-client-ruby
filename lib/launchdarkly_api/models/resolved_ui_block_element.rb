  ...
  class ResolvedUIBlockElement
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `LaunchDarklyApi::ResolvedUIBlockElement` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `LaunchDarklyApi::ResolvedUIBlockElement`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }

      if attributes.key?(:'is_bold')
        self.is_bold = attributes[:'is_bold']
      end
      ...
      if attributes.key?(:'is_timestamp')
        self.is_timestamp = attributes[:'is_timestamp']
      end
    end
    ...
    def hash
      [is_bold, text, url, is_timestamp].hash
    end
