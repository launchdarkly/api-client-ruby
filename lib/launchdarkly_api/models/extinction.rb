  ...
  class Extinction < ApiModelBase
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `LaunchDarklyApi::Extinction` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `LaunchDarklyApi::Extinction`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def list_invalid_properties
      ...
      if @proj_key.nil?
        invalid_properties.push('invalid value for "proj_key", proj_key cannot be nil.')
      end
    ...
    # Custom attribute writer method with validation
    # @param [Object] proj_key Value to be assigned
    def proj_key=(proj_key)
      if proj_key.nil?
        fail ArgumentError, 'proj_key cannot be nil'
      end
    ...
    def hash
      [revision, message, time, flag_key, proj_key].hash
    end
