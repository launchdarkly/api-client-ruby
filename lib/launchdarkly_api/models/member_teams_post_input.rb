...
module LaunchDarklyApi
  class MemberTeamsPostInput
    ...
    def initialize(attributes = {})
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `LaunchDarklyApi::MemberTeamsPostInput`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    # @param [Object] team_keys Value to be assigned
    def team_keys=(team_keys)
      if team_keys.nil?
        fail ArgumentError, 'team_keys cannot be nil'
      end
