...
module LaunchDarklyApi
  class MetricInMetricGroupInput < ApiModelBase
    # The metric key
    attr_accessor :key
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `LaunchDarklyApi::MetricInMetricGroupInput` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `LaunchDarklyApi::MetricInMetricGroupInput`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def list_invalid_properties
      ...
      if @name_in_group.nil?
        invalid_properties.push('invalid value for "name_in_group", name_in_group cannot be nil.')
      end
    ...
    def valid?
      ...
      return false if @name_in_group.nil?
      true
    end
    ...
    # Custom attribute writer method with validation
    # @param [Object] name_in_group Value to be assigned
    def name_in_group=(name_in_group)
      if name_in_group.nil?
        fail ArgumentError, 'name_in_group cannot be nil'
      end

      @name_in_group = name_in_group
    end
