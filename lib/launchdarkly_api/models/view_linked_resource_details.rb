...
module LaunchDarklyApi
  class ViewLinkedResourceDetails < ApiModelBase
    attr_accessor :view
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `LaunchDarklyApi::ViewLinkedResourceDetails` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `LaunchDarklyApi::ViewLinkedResourceDetails`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'ai_config')
        self.ai_config = attributes[:'ai_config']
      end
    ...
    def hash
      [view, flag, segment, ai_config, metric].hash
    end
