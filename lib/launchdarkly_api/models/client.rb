...
module LaunchDarklyApi
  class Client < ApiModelBase
    ...
    def initialize(attributes = {})
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `LaunchDarklyApi::Client`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    # Custom attribute writer method with validation
    # @param [Object] _client_id Value to be assigned
    def _client_id=(_client_id)
      if _client_id.nil?
        fail ArgumentError, '_client_id cannot be nil'
      end
    ...
    def hash
      [_links, name, description, _account_id, _client_id, _client_secret, redirect_uri, _creation_date].hash
    end
