...
module LaunchDarklyApi
  class ConditionInput < ApiModelBase
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `LaunchDarklyApi::ConditionInput` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `LaunchDarklyApi::ConditionInput`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'execute_now')
        self.execute_now = attributes[:'execute_now']
      end
      ...
      if attributes.key?(:'integration_config')
        if (value = attributes[:'integration_config']).is_a?(Hash)
    ...
    def hash
      [schedule_kind, execution_date, wait_duration, wait_duration_unit, execute_now, description, notify_member_ids, notify_team_keys, integration_config, kind].hash
    end
