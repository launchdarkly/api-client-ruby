...
module LaunchDarklyApi
  class InsightsChartMetric < ApiModelBase
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `LaunchDarklyApi::InsightsChartMetric` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `LaunchDarklyApi::InsightsChartMetric`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    # Custom attribute writer method with validation
    # @param [Object] tiers Value to be assigned
    def tiers=(tiers)
      if tiers.nil?
        fail ArgumentError, 'tiers cannot be nil'
      end
    ...
    def hash
      [indicator, value, unit, modifier, tiers].hash
    end
