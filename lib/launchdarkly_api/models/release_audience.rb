...
module LaunchDarklyApi
  class ReleaseAudience < ApiModelBase
    # The audience ID
    attr_accessor :_id
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `LaunchDarklyApi::ReleaseAudience` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `LaunchDarklyApi::ReleaseAudience`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'segment_keys')
        if (value = attributes[:'segment_keys']).is_a?(Array)
          self.segment_keys = value
        end
      end
      ...
      if attributes.key?(:'_rule_ids')
        if (value = attributes[:'_rule_ids']).is_a?(Array)
    ...
    def ==(o)
      ...
      self.class == o.class &&
          ...
          segment_keys == o.segment_keys &&
          status == o.status &&
          _rule_ids == o._rule_ids
    end
    ...
    def hash
      [_id, _links, environment, name, configuration, segment_keys, status, _rule_ids].hash
    end
