...
module LaunchDarklyApi
  class FlagImportStatus < ApiModelBase
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `LaunchDarklyApi::FlagImportStatus` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `LaunchDarklyApi::FlagImportStatus`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def valid?
      ...
      status_validator = EnumAttributeValidator.new('String', ["complete", "importing", "pending", "failed", "partial"])
    ...
    def status=(status)
      validator = EnumAttributeValidator.new('String', ["complete", "importing", "pending", "failed", "partial"])
    ...
    def hash
      [status, last_import, last_error, errors].hash
    end
