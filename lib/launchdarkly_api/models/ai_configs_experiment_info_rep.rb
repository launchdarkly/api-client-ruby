...
module LaunchDarklyApi
  class AiConfigsExperimentInfoRep < ApiModelBase
    attr_accessor :baseline_idx
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `LaunchDarklyApi::AiConfigsExperimentInfoRep` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `LaunchDarklyApi::AiConfigsExperimentInfoRep`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }

      if attributes.key?(:'baseline_idx')
        self.baseline_idx = attributes[:'baseline_idx']
      else
        self.baseline_idx = nil
      end
    ...
    def list_invalid_properties
      ...
      if @baseline_idx.nil?
        invalid_properties.push('invalid value for "baseline_idx", baseline_idx cannot be nil.')
      end
    ...
    def valid?
      ...
      return false if @baseline_idx.nil?
      return false if @items.nil?
      true
    end

    # Custom attribute writer method with validation
    # @param [Object] baseline_idx Value to be assigned
    def baseline_idx=(baseline_idx)
      if baseline_idx.nil?
        fail ArgumentError, 'baseline_idx cannot be nil'
      end
