...
module LaunchDarklyApi
  class Experiment
    ...
    def initialize(attributes = {})
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `LaunchDarklyApi::Experiment`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def hash
      [_id, key, name, description, _maintainer_id, _creation_date, environment_key, archived_date, _links, current_iteration, draft_iteration, previous_iterations].hash
    end
