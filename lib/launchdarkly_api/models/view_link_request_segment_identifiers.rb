...
module LaunchDarklyApi
  class ViewLinkRequestSegmentIdentifiers < ApiModelBase
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `LaunchDarklyApi::ViewLinkRequestSegmentIdentifiers` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `LaunchDarklyApi::ViewLinkRequestSegmentIdentifiers`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }

      if attributes.key?(:'segment_identifiers')
        if (value = attributes[:'segment_identifiers']).is_a?(Array)
          self.segment_identifiers = value
        end
      else
    ...
    def list_invalid_properties
      ...
      if @segment_identifiers.nil?
        invalid_properties.push('invalid value for "segment_identifiers", segment_identifiers cannot be nil.')
      end
    ...
    def valid?
      ...
      return false if @segment_identifiers.nil?
      true
    end

    # Custom attribute writer method with validation
    # @param [Object] segment_identifiers Value to be assigned
    def segment_identifiers=(segment_identifiers)
      if segment_identifiers.nil?
        fail ArgumentError, 'segment_identifiers cannot be nil'
      end
