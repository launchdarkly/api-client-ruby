...
module LaunchDarklyApi
  class AIConfigPost < ApiModelBase
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `LaunchDarklyApi::AIConfigPost` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `LaunchDarklyApi::AIConfigPost`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'view_keys')
        if (value = attributes[:'view_keys']).is_a?(Array)
          self.view_keys = value
        end
      end
      ...
      if attributes.key?(:'evaluation_metric_key')
        self.evaluation_metric_key = attributes[:'evaluation_metric_key']
      end
    ...
    def valid?
      ...
      mode_validator = EnumAttributeValidator.new('String', ["agent", "completion", "judge"])
    ...
    def mode=(mode)
      validator = EnumAttributeValidator.new('String', ["agent", "completion", "judge"])
    ...
    def hash
      [description, key, maintainer_id, maintainer_team_key, mode, name, tags, view_keys, default_variation, evaluation_metric_key, is_inverted].hash
    end
