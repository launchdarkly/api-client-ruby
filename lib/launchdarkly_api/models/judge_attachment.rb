...
module LaunchDarklyApi
  class JudgeAttachment < ApiModelBase
    # Key of the judge AI Config
    attr_accessor :judge_config_key
    ...
    def self.attribute_map
      {
        :'judge_config_key' => :'judgeConfigKey',
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `LaunchDarklyApi::JudgeAttachment` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `LaunchDarklyApi::JudgeAttachment`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }

      if attributes.key?(:'judge_config_key')
        self.judge_config_key = attributes[:'judge_config_key']
      else
        self.judge_config_key = nil
      end

      if attributes.key?(:'sampling_rate')
        self.sampling_rate = attributes[:'sampling_rate']
      else
        self.sampling_rate = nil
      end
    end
    ...
    def list_invalid_properties
      ...
      if @judge_config_key.nil?
        invalid_properties.push('invalid value for "judge_config_key", judge_config_key cannot be nil.')
      end
      ...
      if @sampling_rate > 1
        invalid_properties.push('invalid value for "sampling_rate", must be smaller than or equal to 1.')
      end

      if @sampling_rate < 0
        invalid_properties.push('invalid value for "sampling_rate", must be greater than or equal to 0.')
      end
    ...
    def valid?
      ...
      return false if @judge_config_key.nil?
      return false if @sampling_rate.nil?
      return false if @sampling_rate > 1
      return false if @sampling_rate < 0
      true
    end

    # Custom attribute writer method with validation
    # @param [Object] judge_config_key Value to be assigned
    def judge_config_key=(judge_config_key)
      if judge_config_key.nil?
        fail ArgumentError, 'judge_config_key cannot be nil'
      end
    ...
    def sampling_rate=(sampling_rate)
      ...
      if sampling_rate > 1
        fail ArgumentError, 'invalid value for "sampling_rate", must be smaller than or equal to 1.'
      end

      if sampling_rate < 0
        fail ArgumentError, 'invalid value for "sampling_rate", must be greater than or equal to 0.'
      end
    ...
    def ==(o)
      ...
      self.class == o.class &&
          judge_config_key == o.judge_config_key &&
    ...
    def hash
      [judge_config_key, sampling_rate].hash
    end
