...
module LaunchDarklyApi
  class PatchSegmentExpiringTargetInstruction < ApiModelBase
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `LaunchDarklyApi::PatchSegmentExpiringTargetInstruction` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `LaunchDarklyApi::PatchSegmentExpiringTargetInstruction`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def valid?
      ...
      kind_validator = EnumAttributeValidator.new('String', ["addExpiringTarget", "updateExpiringTarget", "removeExpiringTarget"])
      return false unless kind_validator.valid?(@kind)
      return false if @context_key.nil?
      return false if @context_kind.nil?
      return false if @target_type.nil?
      target_type_validator = EnumAttributeValidator.new('String', ["included", "excluded"])
    ...
    def kind=(kind)
      validator = EnumAttributeValidator.new('String', ["addExpiringTarget", "updateExpiringTarget", "removeExpiringTarget"])
    ...
    def hash
      [kind, context_key, context_kind, target_type, value, version].hash
    end
