  ...
  class MigrationSettingsPost
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `LaunchDarklyApi::MigrationSettingsPost` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `LaunchDarklyApi::MigrationSettingsPost`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'stage_count')
        self.stage_count = attributes[:'stage_count']
      else
        self.stage_count = nil
      end
    end
    ...
    def list_invalid_properties
      ...
      if @stage_count.nil?
        invalid_properties.push('invalid value for "stage_count", stage_count cannot be nil.')
      end
    ...
    def valid?
      ...
      return false if @stage_count.nil?
      true
    end

    # Custom attribute writer method with validation
    # @param [Object] stage_count Value to be assigned
    def stage_count=(stage_count)
      if stage_count.nil?
        fail ArgumentError, 'stage_count cannot be nil'
      end
