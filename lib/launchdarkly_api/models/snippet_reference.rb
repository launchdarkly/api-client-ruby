...
module LaunchDarklyApi
  ...
  class SnippetReference < ApiModelBase
    # The key of the config with a variation that references this snippet.
    attr_accessor :ai_config_key

    # The name of the config with a variation that references this snippet.
    attr_accessor :ai_config_name

    # The ID of the variation that references this snippet.
    attr_accessor :variation_id

    # The key of the config variation that references this snippet.
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `LaunchDarklyApi::SnippetReference` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `LaunchDarklyApi::SnippetReference`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
