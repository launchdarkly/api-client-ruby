...
module LaunchDarklyApi
  class PermissionGrantInput < ApiModelBase
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `LaunchDarklyApi::PermissionGrantInput` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `LaunchDarklyApi::PermissionGrantInput`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }

      if attributes.key?(:'action_set')
        self.action_set = attributes[:'action_set']
      end
    ...
    def valid?
      ...
      action_set_validator = EnumAttributeValidator.new('String', ["maintainTeam"])
      return false unless action_set_validator.valid?(@action_set)
      true
    end

    # Custom attribute writer method checking allowed values (enum).
    # @param [Object] action_set Object to be assigned
    def action_set=(action_set)
      validator = EnumAttributeValidator.new('String', ["maintainTeam"])
      unless validator.valid?(action_set)
        fail ArgumentError, "invalid value for \"action_set\", must be one of #{validator.allowable_values}."
      end
    ...
    def hash
      [action_set, actions, member_ids].hash
    end
