  ...
  class User
    ...
    def self.openapi_types
      {
        ...
        :'derived' => :'Hash<String, DerivedAttribute>',
        :'private_attrs' => :'Array<String>'
      }
    end
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `LaunchDarklyApi::User` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `LaunchDarklyApi::User`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'derived')
        if (value = attributes[:'derived']).is_a?(Hash)
          self.derived = value
        end
      end

      if attributes.key?(:'private_attrs')
        if (value = attributes[:'private_attrs']).is_a?(Array)
          self.private_attrs = value
        end
      end
    end
    ...
    def hash
      [key, secondary, ip, country, email, first_name, last_name, avatar, name, anonymous, custom, derived, private_attrs].hash
    end
