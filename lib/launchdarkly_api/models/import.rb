  ...
  class Import < ApiModelBase
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `LaunchDarklyApi::Import` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `LaunchDarklyApi::Import`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'segment_key')
        self.segment_key = attributes[:'segment_key']
      else
        self.segment_key = nil
      end
    ...
    def list_invalid_properties
      ...
      if @segment_key.nil?
        invalid_properties.push('invalid value for "segment_key", segment_key cannot be nil.')
      end
    ...
    def valid?
      ...
      return false if @segment_key.nil?
      return false if @creation_time.nil?
      return false if @mode.nil?
      return false if @status.nil?
      status_validator = EnumAttributeValidator.new('String', ["preparing", "pending_approval", "ready", "in_progress", "complete", "stopped"])
    ...
    # Custom attribute writer method with validation
    # @param [Object] segment_key Value to be assigned
    def segment_key=(segment_key)
      if segment_key.nil?
        fail ArgumentError, 'segment_key cannot be nil'
      end
    ...
    def status=(status)
      validator = EnumAttributeValidator.new('String', ["preparing", "pending_approval", "ready", "in_progress", "complete", "stopped"])
    ...
    def hash
      [id, segment_key, creation_time, mode, status, files, _links].hash
    end
