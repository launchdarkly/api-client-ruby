...
module LaunchDarklyApi
  class ProjectPost < ApiModelBase
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `LaunchDarklyApi::ProjectPost` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `LaunchDarklyApi::ProjectPost`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'naming_convention')
        self.naming_convention = attributes[:'naming_convention']
      end
    end
    ...
    def ==(o)
      ...
      self.class == o.class &&
          ...
          naming_convention == o.naming_convention
    end
    ...
    def hash
      [name, key, include_in_snippet_by_default, default_client_side_availability, tags, environments, naming_convention].hash
    end
