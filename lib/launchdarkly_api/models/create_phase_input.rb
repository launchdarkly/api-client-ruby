...
module LaunchDarklyApi
  class CreatePhaseInput < ApiModelBase
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `LaunchDarklyApi::CreatePhaseInput` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `LaunchDarklyApi::CreatePhaseInput`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def list_invalid_properties
      ...
      if @audiences.nil?
        invalid_properties.push('invalid value for "audiences", audiences cannot be nil.')
      end
    ...
    def valid?
      ...
      return false if @audiences.nil?
      return false if @name.nil?
      true
    end

    # Custom attribute writer method with validation
    # @param [Object] audiences Value to be assigned
    def audiences=(audiences)
      if audiences.nil?
        fail ArgumentError, 'audiences cannot be nil'
      end
    ...
    def hash
      [audiences, name, configuration].hash
    end
