  ...
  class BayesianBetaBinomialStatsRep
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `LaunchDarklyApi::BayesianBetaBinomialStatsRep` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `LaunchDarklyApi::BayesianBetaBinomialStatsRep`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }

      if attributes.key?(:'prior_alpha')
        self.prior_alpha = attributes[:'prior_alpha']
      end

      if attributes.key?(:'prior_beta')
        self.prior_beta = attributes[:'prior_beta']
      end

      if attributes.key?(:'prior_mean')
        self.prior_mean = attributes[:'prior_mean']
      end

      if attributes.key?(:'data_weight')
        self.data_weight = attributes[:'data_weight']
      end
    end
    ...
    def ==(o)
      ...
      self.class == o.class &&
          prior_alpha == o.prior_alpha &&
          prior_beta == o.prior_beta &&
          prior_mean == o.prior_mean &&
          data_weight == o.data_weight
    end
    ...
    def hash
      [prior_alpha, prior_beta, prior_mean, data_weight].hash
    end
