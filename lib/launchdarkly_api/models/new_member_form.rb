...
module LaunchDarklyApi
  class NewMemberForm < ApiModelBase
    # The member's email
    attr_accessor :email
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `LaunchDarklyApi::NewMemberForm` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `LaunchDarklyApi::NewMemberForm`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def valid?
      ...
      role_validator = EnumAttributeValidator.new('String', ["reader", "writer", "admin", "no_access"])
    ...
    def role=(role)
      validator = EnumAttributeValidator.new('String', ["reader", "writer", "admin", "no_access"])
    ...
    def hash
      [email, password, first_name, last_name, role, custom_roles, team_keys, role_attributes].hash
    end
