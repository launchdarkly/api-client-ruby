  ...
  class SdkListRep < ApiModelBase
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `LaunchDarklyApi::SdkListRep` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `LaunchDarklyApi::SdkListRep`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'sdks')
        if (value = attributes[:'sdks']).is_a?(Array)
          self.sdks = value
        end
      else
        self.sdks = nil
    ...
    def list_invalid_properties
      ...
      if @sdks.nil?
        invalid_properties.push('invalid value for "sdks", sdks cannot be nil.')
      end
    ...
    # Custom attribute writer method with validation
    # @param [Object] sdks Value to be assigned
    def sdks=(sdks)
      if sdks.nil?
        fail ArgumentError, 'sdks cannot be nil'
      end
