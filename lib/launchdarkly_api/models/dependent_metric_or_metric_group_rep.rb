...
module LaunchDarklyApi
  class DependentMetricOrMetricGroupRep < ApiModelBase
    ...
    # The context kind chosen as the analysis unit for this metric in the iteration. If null, analysis uses the iteration's randomization unit.
    ...
    def initialize(attributes = {})
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `LaunchDarklyApi::DependentMetricOrMetricGroupRep`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def valid?
      ...
      kind_validator = EnumAttributeValidator.new('String', ["pageview", "click", "custom", "trace", "funnel", "standard"])
    ...
    def kind=(kind)
      validator = EnumAttributeValidator.new('String', ["pageview", "click", "custom", "trace", "funnel", "standard"])
    ...
    def hash
      [key, _version_id, name, kind, is_numeric, event_key, _links, is_group, metrics, analysis_type, analysis_unit].hash
    end
