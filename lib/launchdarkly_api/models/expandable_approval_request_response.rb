...
module LaunchDarklyApi
  class ExpandableApprovalRequestResponse < ApiModelBase
    ...
    attr_accessor :applied_by_service_token_id
    ...
    attr_accessor :integration_metadata
    ...
    attr_accessor :approval_settings
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `LaunchDarklyApi::ExpandableApprovalRequestResponse` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `LaunchDarklyApi::ExpandableApprovalRequestResponse`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'applied_by_service_token_id')
        self.applied_by_service_token_id = attributes[:'applied_by_service_token_id']
      end
    ...
    def valid?
      ...
      review_status_validator = EnumAttributeValidator.new('String', ["approved", "declined", "pending"])
      ...
      status_validator = EnumAttributeValidator.new('String', ["pending", "completed", "failed", "scheduled"])
    ...
    def review_status=(review_status)
      validator = EnumAttributeValidator.new('String', ["approved", "declined", "pending"])
    ...
    # Custom attribute writer method with validation
    # @param [Object] all_reviews Value to be assigned
    def all_reviews=(all_reviews)
      if all_reviews.nil?
        fail ArgumentError, 'all_reviews cannot be nil'
      end
    ...
    # Custom attribute writer method with validation
    # @param [Object] notify_member_ids Value to be assigned
    def notify_member_ids=(notify_member_ids)
      if notify_member_ids.nil?
        fail ArgumentError, 'notify_member_ids cannot be nil'
      end

      @notify_member_ids = notify_member_ids
    end

    # Custom attribute writer method checking allowed values (enum).
    # @param [Object] status Object to be assigned
    def status=(status)
      validator = EnumAttributeValidator.new('String', ["pending", "completed", "failed", "scheduled"])
    ...
    def conflicts=(conflicts)
      if conflicts.nil?
        fail ArgumentError, 'conflicts cannot be nil'
      end
    ...
    def ==(o)
      ...
      self.class == o.class &&
          ...
          applied_by_service_token_id == o.applied_by_service_token_id &&
          status == o.status &&
    ...
    def hash
      [_id, _version, creation_date, service_kind, requestor_id, description, review_status, all_reviews, notify_member_ids, applied_date, applied_by_member_id, applied_by_service_token_id, status, instructions, conflicts, _links, execution_date, operating_on_id, integration_metadata, source, custom_workflow_metadata, resource_id, approval_settings, project, environments, flag, resource].hash
    end
