...
module LaunchDarklyApi
  class ApplicationRep < ApiModelBase
    attr_accessor :flags
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `LaunchDarklyApi::ApplicationRep` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `LaunchDarklyApi::ApplicationRep`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def valid?
      ...
      kind_validator = EnumAttributeValidator.new('String', ["browser", "mobile", "server"])
    ...
    # @param [Object] auto_added Value to be assigned
    def auto_added=(auto_added)
      if auto_added.nil?
        fail ArgumentError, 'auto_added cannot be nil'
      end
    ...
    def kind=(kind)
      validator = EnumAttributeValidator.new('String', ["browser", "mobile", "server"])
    ...
    def hash
      [flags, _access, _links, _version, auto_added, creation_date, description, key, kind, _maintainer, name].hash
    end
