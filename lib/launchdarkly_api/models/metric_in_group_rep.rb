...
module LaunchDarklyApi
  class MetricInGroupRep < ApiModelBase
    # The metric key
    attr_accessor :key
    ...
    # The context kind used as the analysis unit for this metric in the experiment iteration
    attr_accessor :analysis_unit
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `LaunchDarklyApi::MetricInGroupRep` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `LaunchDarklyApi::MetricInGroupRep`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'analysis_unit')
        self.analysis_unit = attributes[:'analysis_unit']
      end
      ...
      if attributes.key?(:'name_in_group')
        self.name_in_group = attributes[:'name_in_group']
      end
    ...
    def valid?
      ...
      unit_aggregation_type_validator = EnumAttributeValidator.new('String', ["sum", "average", "count_distinct"])
    ...
    def unit_aggregation_type=(unit_aggregation_type)
      validator = EnumAttributeValidator.new('String', ["sum", "average", "count_distinct"])
    ...
    def ==(o)
      ...
      self.class == o.class &&
          ...
          analysis_unit == o.analysis_unit &&
          _links == o._links &&
          name_in_group == o.name_in_group &&
    ...
    def hash
      [key, _version_id, name, kind, is_numeric, unit_aggregation_type, analysis_type, event_key, analysis_unit, _links, name_in_group, randomization_units, analysis_units].hash
    end
