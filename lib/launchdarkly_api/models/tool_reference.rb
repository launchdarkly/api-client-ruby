...
module LaunchDarklyApi
  ...
  class ToolReference < ApiModelBase
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `LaunchDarklyApi::ToolReference` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `LaunchDarklyApi::ToolReference`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    # Custom attribute writer method with validation
    # @param [Object] tool_version Value to be assigned
    def tool_version=(tool_version)
      if tool_version.nil?
        fail ArgumentError, 'tool_version cannot be nil'
      end
    ...
    def hash
      [ai_config_key, ai_config_name, variation_id, variation_key, variation_name, tool_version].hash
    end
