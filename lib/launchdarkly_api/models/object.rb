  ...
  class Object
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `LaunchDarklyApi::Object` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `LaunchDarklyApi::Object`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'pkg_path')
        self.pkg_path = attributes[:'pkg_path']
      end

      if attributes.key?(:'pkg_name')
        self.pkg_name = attributes[:'pkg_name']
      end

      if attributes.key?(:'valid_pos')
        self.valid_pos = attributes[:'valid_pos']
      end

      if attributes.key?(:'shadow')
        if (value = attributes[:'shadow']).is_a?(Hash)
          self.shadow = value
        end
      end
    end
    ...
    def hash
      [name, kind, pkg_path, pkg_name, valid_pos, shadow].hash
    end
