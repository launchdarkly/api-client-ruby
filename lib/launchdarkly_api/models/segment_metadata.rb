...
module LaunchDarklyApi
  class SegmentMetadata < ApiModelBase
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `LaunchDarklyApi::SegmentMetadata` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `LaunchDarklyApi::SegmentMetadata`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'excluded_count')
        self.excluded_count = attributes[:'excluded_count']
      end
    ...
    def ==(o)
      ...
      self.class == o.class &&
          ...
          excluded_count == o.excluded_count &&
    ...
    def hash
      [env_id, segment_id, version, included_count, excluded_count, last_modified, deleted].hash
    end
