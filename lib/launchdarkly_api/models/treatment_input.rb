...
module LaunchDarklyApi
  class TreatmentInput < ApiModelBase
    # The treatment name
    attr_accessor :name
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `LaunchDarklyApi::TreatmentInput` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `LaunchDarklyApi::TreatmentInput`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def list_invalid_properties
      ...
      if @allocation_percent.nil?
        invalid_properties.push('invalid value for "allocation_percent", allocation_percent cannot be nil.')
      end
    ...
    def valid?
      ...
      return false if @allocation_percent.nil?
    ...
    # Custom attribute writer method with validation
    # @param [Object] baseline Value to be assigned
    def baseline=(baseline)
      if baseline.nil?
        fail ArgumentError, 'baseline cannot be nil'
      end
    ...
    def hash
      [name, baseline, allocation_percent, parameters].hash
    end
