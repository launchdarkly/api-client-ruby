...
module LaunchDarklyApi
  class FormVariable < ApiModelBase
    attr_accessor :key
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `LaunchDarklyApi::FormVariable` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `LaunchDarklyApi::FormVariable`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'dynamic_options')
        self.dynamic_options = attributes[:'dynamic_options']
      end
    end
    ...
    def ==(o)
      ...
      self.class == o.class &&
          ...
          dynamic_options == o.dynamic_options
    end
    ...
    def hash
      [key, name, type, description, placeholder, is_optional, default_value, allowed_values, dynamic_options].hash
    end
