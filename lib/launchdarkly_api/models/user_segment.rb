...
module LaunchDarklyApi
  class UserSegment < ApiModelBase
    ...
    def initialize(attributes = {})
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `LaunchDarklyApi::UserSegment`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    # Custom attribute writer method with validation
    # @param [Object] last_modified_date Value to be assigned
    def last_modified_date=(last_modified_date)
      if last_modified_date.nil?
        fail ArgumentError, 'last_modified_date cannot be nil'
      end
    ...
    def hash
      [name, description, tags, creation_date, last_modified_date, key, included, excluded, included_contexts, excluded_contexts, _links, rules, version, deleted, _access, _flags, unbounded, unbounded_context_kind, generation, _unbounded_metadata, _external, _external_link, _import_in_progress].hash
    end
