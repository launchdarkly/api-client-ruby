...
module LaunchDarklyApi
  class UserSegment
    ...
    # An array of keys for included targets. Included individual targets are always segment members, regardless of segment rules. For Big Segments this array is either empty or omitted.
    attr_accessor :included

    # An array of keys for excluded targets. Segment rules bypass individual excluded targets, so they will never be included based on rules. Excluded targets may still be included explicitly. This value is omitted for Big Segments.
    attr_accessor :excluded

    attr_accessor :included_contexts

    attr_accessor :excluded_contexts
    ...
    def initialize(attributes = {})
      ...
      if attributes.key?(:'_unbounded_metadata')
        self._unbounded_metadata = attributes[:'_unbounded_metadata']
      end
    ...
    def hash
      [name, description, tags, creation_date, key, included, excluded, included_contexts, excluded_contexts, _links, rules, version, deleted, _access, _flags, unbounded, unbounded_context_kind, generation, _unbounded_metadata, _external, _external_link, _import_in_progress].hash
    end
