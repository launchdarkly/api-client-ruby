...
module LaunchDarklyApi
  class AuditLogEntryListingRep < ApiModelBase
    ...
    def initialize(attributes = {})
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `LaunchDarklyApi::AuditLogEntryListingRep`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def hash
      [_links, _id, _account_id, date, accesses, kind, name, description, short_description, comment, related_flag, related_experiment, related_ai_config, subject, member, token, app, title_verb, title, target, parent].hash
    end
