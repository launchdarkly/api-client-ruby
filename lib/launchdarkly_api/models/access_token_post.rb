...
module LaunchDarklyApi
  class AccessTokenPost < ApiModelBase
    ...
    def self.openapi_types
      {
        ...
        :'inline_role' => :'Array<StatementPost>',
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `LaunchDarklyApi::AccessTokenPost` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `LaunchDarklyApi::AccessTokenPost`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def valid?
      ...
      role_validator = EnumAttributeValidator.new('String', ["reader", "writer", "admin"])
    ...
    def role=(role)
      validator = EnumAttributeValidator.new('String', ["reader", "writer", "admin"])
    ...
    def hash
      [name, description, role, custom_role_ids, inline_role, service_token, default_api_version].hash
    end
