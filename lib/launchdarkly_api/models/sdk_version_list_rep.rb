...
module LaunchDarklyApi
  class SdkVersionListRep < ApiModelBase
    ...
    def self.openapi_types
      {
        ...
        :'sdk_versions' => :'Array<SdkVersionRep>'
      }
    end
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `LaunchDarklyApi::SdkVersionListRep` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `LaunchDarklyApi::SdkVersionListRep`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'sdk_versions')
        if (value = attributes[:'sdk_versions']).is_a?(Array)
          self.sdk_versions = value
        end
      else
        self.sdk_versions = nil
      end
    end
    ...
    def list_invalid_properties
      ...
      if @sdk_versions.nil?
        invalid_properties.push('invalid value for "sdk_versions", sdk_versions cannot be nil.')
      end
    ...
    def valid?
      ...
      return false if @sdk_versions.nil?
      true
    end
    ...
    # Custom attribute writer method with validation
    # @param [Object] sdk_versions Value to be assigned
    def sdk_versions=(sdk_versions)
      if sdk_versions.nil?
        fail ArgumentError, 'sdk_versions cannot be nil'
      end
