...
module LaunchDarklyApi
  class PhaseConfiguration
    # The bake time duration in milliseconds
    attr_accessor :bake_time_duration_ms

    # Attribute mapping from ruby-style variable name to JSON key.
    def self.attribute_map
      {
        :'bake_time_duration_ms' => :'bakeTimeDurationMs'
      }
    end
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `LaunchDarklyApi::PhaseConfiguration` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `LaunchDarklyApi::PhaseConfiguration`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }

      if attributes.key?(:'bake_time_duration_ms')
        self.bake_time_duration_ms = attributes[:'bake_time_duration_ms']
      end
    end
    ...
    def ==(o)
      ...
      self.class == o.class &&
          bake_time_duration_ms == o.bake_time_duration_ms
    end
