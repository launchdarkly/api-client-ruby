...
module LaunchDarklyApi
  class AIConfigVariationPatch < ApiModelBase
    ...
    def self.openapi_types
      {
        ...
        :'published' => :'Boolean',
        :'state' => :'String',
        :'tools' => :'Array<VariationToolPost>',
        :'tool_keys' => :'Array<String>',
        :'skills' => :'Array<VariationSkillPost>',
        :'judge_configuration' => :'JudgeConfiguration'
      }
    end
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `LaunchDarklyApi::AIConfigVariationPatch` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `LaunchDarklyApi::AIConfigVariationPatch`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'tool_keys')
        if (value = attributes[:'tool_keys']).is_a?(Array)
          self.tool_keys = value
        end
      end
    ...
    def hash
      [comment, description, instructions, messages, model, model_config_key, name, output_format, published, state, tools, tool_keys, skills, judge_configuration].hash
    end
