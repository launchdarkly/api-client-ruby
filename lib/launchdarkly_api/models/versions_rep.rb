...
module LaunchDarklyApi
  class VersionsRep < ApiModelBase
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `LaunchDarklyApi::VersionsRep` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `LaunchDarklyApi::VersionsRep`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }

      if attributes.key?(:'valid_versions')
        if (value = attributes[:'valid_versions']).is_a?(Array)
          self.valid_versions = value
        end
      else
        self.valid_versions = nil
      end
    ...
    def list_invalid_properties
      ...
      if @valid_versions.nil?
        invalid_properties.push('invalid value for "valid_versions", valid_versions cannot be nil.')
      end
    ...
    def valid?
      ...
      return false if @valid_versions.nil?
    ...
    # @param [Object] valid_versions Value to be assigned
    def valid_versions=(valid_versions)
      if valid_versions.nil?
        fail ArgumentError, 'valid_versions cannot be nil'
      end
    ...
    def ==(o)
      ...
      self.class == o.class &&
          valid_versions == o.valid_versions &&
    ...
    def hash
      [valid_versions, latest_version, current_version, beta].hash
    end
