...
module LaunchDarklyApi
  class CreateCopyFlagConfigApprovalRequestRequest < ApiModelBase
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `LaunchDarklyApi::CreateCopyFlagConfigApprovalRequestRequest` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `LaunchDarklyApi::CreateCopyFlagConfigApprovalRequestRequest`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'included_actions')
        if (value = attributes[:'included_actions']).is_a?(Array)
          self.included_actions = value
        end
      end

      if attributes.key?(:'excluded_actions')
        if (value = attributes[:'excluded_actions']).is_a?(Array)
    ...
    def hash
      [comment, description, notify_member_ids, notify_team_keys, source, included_actions, excluded_actions].hash
    end
