...
module LaunchDarklyApi
  class BigSegmentStoreIntegration < ApiModelBase
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `LaunchDarklyApi::BigSegmentStoreIntegration` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `LaunchDarklyApi::BigSegmentStoreIntegration`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def valid?
      ...
      integration_key_validator = EnumAttributeValidator.new('String', ["redis", "dynamodb"])
      return false unless integration_key_validator.valid?(@integration_key)
    ...
    # @param [Object] integration_key Object to be assigned
    def integration_key=(integration_key)
      validator = EnumAttributeValidator.new('String', ["redis", "dynamodb"])
      unless validator.valid?(integration_key)
        fail ArgumentError, "invalid value for \"integration_key\", must be one of #{validator.allowable_values}."
      end
    ...
    # Custom attribute writer method with validation
    # @param [Object] on Value to be assigned
    def on=(on)
      if on.nil?
        fail ArgumentError, 'on cannot be nil'
      end
    ...
    # Custom attribute writer method with validation
    # @param [Object] _status Value to be assigned
    def _status=(_status)
      if _status.nil?
        fail ArgumentError, '_status cannot be nil'
      end
    ...
    def hash
      [_links, _id, integration_key, project_key, environment_key, config, on, tags, name, version, _access, _status].hash
    end
