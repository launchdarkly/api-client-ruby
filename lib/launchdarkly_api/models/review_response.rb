...
module LaunchDarklyApi
  class ReviewResponse < ApiModelBase
    ...
    attr_accessor :service_token_id
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `LaunchDarklyApi::ReviewResponse` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `LaunchDarklyApi::ReviewResponse`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'service_token_id')
        self.service_token_id = attributes[:'service_token_id']
      end
    end
    ...
    def ==(o)
      ...
      self.class == o.class &&
          ...
          service_token_id == o.service_token_id
    end
    ...
    def hash
      [_id, kind, creation_date, comment, member_id, service_token_id].hash
    end
