...
module LaunchDarklyApi
  class InsightsRepository < ApiModelBase
    # The repository ID
    attr_accessor :_id
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `LaunchDarklyApi::InsightsRepository` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `LaunchDarklyApi::InsightsRepository`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'main_branch')
        self.main_branch = attributes[:'main_branch']
      else
        self.main_branch = nil
      end
    ...
    def list_invalid_properties
      ...
      if @main_branch.nil?
        invalid_properties.push('invalid value for "main_branch", main_branch cannot be nil.')
      end
    ...
    def valid?
      ...
      return false if @main_branch.nil?
      true
    end
    ...
    # Custom attribute writer method with validation
    # @param [Object] main_branch Value to be assigned
    def main_branch=(main_branch)
      if main_branch.nil?
        fail ArgumentError, 'main_branch cannot be nil'
      end
    ...
    def hash
      [_id, version, key, type, url, main_branch, projects].hash
    end
