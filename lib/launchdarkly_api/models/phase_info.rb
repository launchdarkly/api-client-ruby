  ...
  class PhaseInfo < ApiModelBase
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `LaunchDarklyApi::PhaseInfo` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `LaunchDarklyApi::PhaseInfo`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def list_invalid_properties
      ...
      if @release_count.nil?
        invalid_properties.push('invalid value for "release_count", release_count cannot be nil.')
      end
    ...
    def valid?
      ...
      return false if @release_count.nil?
      true
    end
    ...
    # Custom attribute writer method with validation
    # @param [Object] release_count Value to be assigned
    def release_count=(release_count)
      if release_count.nil?
        fail ArgumentError, 'release_count cannot be nil'
      end
