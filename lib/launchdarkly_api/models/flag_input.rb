  ...
  class FlagInput < ApiModelBase
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `LaunchDarklyApi::FlagInput` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `LaunchDarklyApi::FlagInput`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def list_invalid_properties
      ...
      if @flag_config_version.nil?
        invalid_properties.push('invalid value for "flag_config_version", flag_config_version cannot be nil.')
      end
    ...
    def valid?
      ...
      return false if @flag_config_version.nil?
      true
    end
    ...
    # Custom attribute writer method with validation
    # @param [Object] flag_config_version Value to be assigned
    def flag_config_version=(flag_config_version)
      if flag_config_version.nil?
        fail ArgumentError, 'flag_config_version cannot be nil'
      end

      @flag_config_version = flag_config_version
    end
    ...
    def hash
      [rule_id, flag_config_version, not_in_experiment_variation_id].hash
    end
