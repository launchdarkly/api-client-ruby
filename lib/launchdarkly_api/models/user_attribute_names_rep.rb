  ...
  class UserAttributeNamesRep
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `LaunchDarklyApi::UserAttributeNamesRep` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `LaunchDarklyApi::UserAttributeNamesRep`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }

      if attributes.key?(:'private')
        if (value = attributes[:'private']).is_a?(Array)
          self.private = value
        end
      end
      ...
      if attributes.key?(:'standard')
        if (value = attributes[:'standard']).is_a?(Array)
          self.standard = value
        end
      end
    end
