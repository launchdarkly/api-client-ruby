  ...
  class BranchRep < ApiModelBase
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `LaunchDarklyApi::BranchRep` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `LaunchDarklyApi::BranchRep`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'update_sequence_id')
        self.update_sequence_id = attributes[:'update_sequence_id']
      end
    ...
    def sync_time=(sync_time)
      if sync_time.nil?
        fail ArgumentError, 'sync_time cannot be nil'
      end
    ...
    def hash
      [name, head, update_sequence_id, sync_time, references, _links].hash
    end
