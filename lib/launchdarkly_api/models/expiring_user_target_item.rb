...
module LaunchDarklyApi
  class ExpiringUserTargetItem < ApiModelBase
    ...
    def self.openapi_types
      {
        ...
        :'target_type' => :'String',
        :'variation_id' => :'String',
        :'_resource_id' => :'ResourceIDResponse'
      }
    end
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `LaunchDarklyApi::ExpiringUserTargetItem` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `LaunchDarklyApi::ExpiringUserTargetItem`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    # Custom attribute writer method with validation
    # @param [Object] user_key Value to be assigned
    def user_key=(user_key)
      if user_key.nil?
        fail ArgumentError, 'user_key cannot be nil'
      end
    ...
    def hash
      [_id, _version, expiration_date, user_key, target_type, variation_id, _resource_id].hash
    end
