...
module LaunchDarklyApi
  class ReleasePoliciesAccessRep < ApiModelBase
    ...
    def self.openapi_types
      {
        :'denied' => :'Array<ReleasePoliciesAccessDenied>',
        :'allowed' => :'Array<ReleasePoliciesAccessAllowedRep>'
      }
    end
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `LaunchDarklyApi::ReleasePoliciesAccessRep` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `LaunchDarklyApi::ReleasePoliciesAccessRep`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }

      if attributes.key?(:'denied')
        if (value = attributes[:'denied']).is_a?(Array)
          self.denied = value
        end
      else
        self.denied = nil
