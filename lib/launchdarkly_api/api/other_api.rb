...
module LaunchDarklyApi
  class OtherApi
    ...
    def get_caller_identity(opts = {})
      data, _status_code, _headers = get_caller_identity_with_http_info(opts)
      data
    end
    ...
    # Get basic information about the identity used (session cookie, API token, SDK keys, etc.) to call the API
    # @param [Hash] opts the optional parameters
    # @return [Array<(CallerIdentityRep, Integer, Hash)>] CallerIdentityRep data, response status code and response headers
    def get_caller_identity_with_http_info(opts = {})
      if @api_client.config.debugging
        @api_client.config.logger.debug 'Calling API: OtherApi.get_caller_identity ...'
      end
      # resource path
      local_var_path = '/api/v2/caller-identity'
      ...
      # return_type
      return_type = opts[:debug_return_type] || 'CallerIdentityRep'

      # auth_names
      auth_names = opts[:debug_auth_names] || ['ApiKey']

      new_options = opts.merge(
        :operation => :"OtherApi.get_caller_identity",
      ...
      if @api_client.config.debugging
        @api_client.config.logger.debug "API called: OtherApi#get_caller_identity\nData: #{data.inspect}\nStatus code: #{status_code}\nHeaders: #{headers}"
      end
    ...
    # Get all of the resource categories the API supports. In the sandbox, click &#39;Play&#39; and enter any string in the &#39;Authorization&#39; field to test this endpoint.
    # @param [Hash] opts the optional parameters
    # @return [Array<(RootResponse, Integer, Hash)>] RootResponse data, response status code and response headers
    def get_root_with_http_info(opts = {})
      ...
      # return_type
      return_type = opts[:debug_return_type] || 'RootResponse'
    ...
    # Get the latest API version, the list of valid API versions in ascending order, and the version being used for this request. These are all in the external, date-based format.
    # @param [Hash] opts the optional parameters
    # @return [Array<(VersionsRep, Integer, Hash)>] VersionsRep data, response status code and response headers
